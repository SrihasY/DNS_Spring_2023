...
def is_model_ready():
	buffer = ''
	try:
		buffer = requests.get(url="http://localhost:10001/models/langdetect")
...
def handler(event, context):
	...
	if('dummy' in event) and (event['dummy'] == 1):
		print("Dummy call for langdetect, doing nothing")
		return {"out":"Dummy call for langdetect, doing nothing"}
